...
export function TaskInput({ onCreate, editingTask }: Props) {
  ...
  return (
    <form onSubmit={submit} className="panel rounded-2xl p-5">
      ...
      <label className="text-xs text-slate-500">Mission Objective</label>
      <input value={form.title} onChange={(e) => setForm((p) => ({ ...p, title: e.target.value }))} placeholder="Define mission objective" className="mt-1 w-full rounded-xl border border-slate-700 bg-slate-900/70 px-3 py-2.5" />
      <label className="mt-3 block text-xs text-slate-500">Tactical Note</label>
      <input value={form.note} onChange={(e) => setForm((p) => ({ ...p, note: e.target.value }))} placeholder="Optional tactical note" className="mt-1 w-full rounded-xl border border-slate-700 bg-slate-900/70 px-3 py-2 text-sm" />
      ...
      <button className="mt-4 rounded-xl border accent-border accent-bg-soft px-4 py-2 text-sm font-semibold shadow-[0_8px_18px_rgba(0,0,0,0.2)] transition hover:-translate-y-0.5">
